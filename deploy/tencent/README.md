...
5. 写入两个独立 secret file（不得放进 release bundle、日志或 Git），再使用 `docker compose -p doccanvas-kgraph -f deploy/tencent/compose.yaml --env-file <release.env> config --quiet` 验证最终变量。
6. 停止写入后运行 `DOCCANVAS_WRITES_QUIESCED=1 scripts/tencent/backup-owner-data.sh <data-root> <backup-dir>`，记录 archive checksum。
