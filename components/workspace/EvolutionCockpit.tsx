...
export function EvolutionCockpit({ projection }: Props) {
  return <div className="operations-workspace evolution-cockpit">
    ...
    <section className="evolution-checks" aria-labelledby="evolution-checks-title"><header><div><span>READINESS MATRIX</span><h2 id="evolution-checks-title">证据刹车</h2></div><p><LockKeyhole aria-hidden="true" />全部状态由 Evidence Registry 派生</p></header><div>{projection.evolution.checks.map(check => <article key={check.id} data-status={check.status} data-state={check.status}><span>{check.status === 'ready' ? '证据就绪' : check.status === 'blocked' ? '证据阻断' : '尚未测量'}</span><strong>{check.label}</strong><p>{check.evidence}</p><code>{check.evidenceIds[0]}</code><small>新鲜度：{check.freshness} · 需要：{check.requiredEvidence}</small></article>)}</div></section>
