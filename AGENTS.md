...
2. 涉及产品 UI、交互、视觉或响应式时，读取 `DESIGN.md` 与 `docs/engineering/governed-workbench-ui-inventory.md`；前者是目标体验契约，后者是当前实现与缺口基线。
3. 涉及画布、Owner 写入或发布时，读取 `docs/engineering/factory-scene-v3.md` 与 `deploy/tencent/PRODUCTION-RUNBOOK.md`。
4. 写入前执行 `git status --short --branch`，保留非本任务改动。
5. 先搜索 `lib/shared/document-registry.ts`、`lib/markdown/sections.ts`、`lib/server/document-mutations.ts` 与相关测试，确认现有契约后再实现。
...
components/ui/                    Design System v2 primitives、统一状态、overlay 与键盘契约
